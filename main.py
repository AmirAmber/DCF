...
class Company:
    def __init__(self, Ticker, API_Key):
        ...
        try:
            ...
            try:  # get implied shares outstanding
                Implied_Shares_Outstanding = statisticsv3.get('defaultKeyStatistics').get(
                    'impliedSharesOutstanding').get('raw')
            except Exception as e:
            ...
            try:
                try:
                    self.Liabilities = financialsv2.get('balanceSheetHistoryQuarterly').get('balanceSheetStatements')[
                        0].get('totalLiab').get('raw')
            ...
            # Revenue from API
            try:
                self.TTM_Revenue = financialsv2.get("timeSeries").get('trailingTotalRevenue')[0].get(
                    'reportedValue').get('raw')
            ...
            try:
                ...
                Error_Dict[str(TTM_year) + " Revenue"] = Error_Dict[str(TTM_1) + " Revenue"] = Error_Dict[
                    str(TTM_2) + " Revenue"] = Error_Dict[str(TTM_3) + " Revenue"] = Error_Dict[
                    str(TTM_4) + " Revenue"] = Error_Dict[str(TTM_5) + " Revenue"] = e
            ...
            # Net income from API
            try:
                self.TTM_Net_Income = financialsv2.get("timeSeries").get('trailingNetIncome')[0].get(
                    'reportedValue').get('raw')
            ...
            try:
                ...
                Error_Dict[str(TTM_year) + " Net Income"] = Error_Dict[str(TTM_1) + " Net Income"] = Error_Dict[
                    str(TTM_2) + " Net Income"] = Error_Dict[
                    str(TTM_3) + " Net Income"] = Error_Dict[str(TTM_4) + " Net Income"] = Error_Dict[
                    str(TTM_5) + " Net Income"] = e
            ...
            try:
                Trailing_Operating_Cash_Flow = (cashflowsv2.get("timeSeries").get('trailingOperatingCashFlow'))[0].get(
                    'reportedValue').get('raw')
            except Exception as e:
                Trailing_Operating_Cash_Flow = 0
            try:
                Trailing_Capex = abs(
                    (cashflowsv2.get("timeSeries").get('trailingCapitalExpenditure'))[0].get('reportedValue').get(
                        'raw'))
            except Exception as e:
                Trailing_Capex = 0
            ...
            try:
                ...
                for index in range(len(Cash_Flow_Statements)):
                    year = str(
                        ((Cash_Flow_Statements[-(index + 1)]).get('endDate').get('fmt'))[0:4]) + " Free Cash Flow"
                    Cash_Flow_From_Operations = (Cash_Flow_Statements[-(index + 1)]).get(
                        'totalCashFromOperatingActivities').get('raw')
                ...
                Error_Dict[str(TTM_year) + " Free Cash Flow"] = Error_Dict[str(TTM_1) + " Free Cash Flow"] = Error_Dict[
                    str(TTM_2) + " Free Cash Flow"] = Error_Dict[str(TTM_3) + " Free Cash Flow"] = Error_Dict[
                    str(TTM_4) + " Free Cash Flow"] = Error_Dict[str(TTM_5) + " Free Cash Flow"] = e
        ...
        try:
            self.Next_Earnings_Date = self.statisticsv3.get('calendarEvents').get('earnings').get('earningsDate')[
                0].get('fmt')  # אפשר לעשות 1 כנתון גיבוי
    ...
    def Save_Company(self):
        ...
        with open(r'C:\Users\amira\OneDrive\Desktop\dfs\companies1_CSV.txt', 'a') as companies_CSV:
            ...
            if self.Error_Dict.get('Valid Ticker') == None:
                csv_data = [self.Ticker, self.Current_Price, self.Dividend, self.Shares_Outstanding, self.Sector,
                            self.Current_Assets, self.Liabilities, self.TTM_Revenue, self._2022Revenue,
                            self._2021Revenue,
        ...
        with open(r'C:\Users\amira\OneDrive\Desktop\dfs\Error_log1.txt', 'a') as Errors_CSV:
            ...
            Order_List = ['Ticker', 'Current Price', 'Dividend', 'Shares Outstanding', 'Sector', 'Current Assets',
                          'Liabilities', 'TTM Revenue', '2022 Revenue', '2021 Revenue', '2020 Revenue', '2019 Revenue',
                          '2018 Revenue', '2017 Revenue', 'TTM+1 Revenue', 'TTM Net Income', '2022 Net Income',
                          '2021 Net Income', '2020 Net Income', '2019 Net Income', '2018 Net Income', '2017 Net Income',
                          'TTM+1 Net Income', 'TTM Free Cash Flow', '2022 Free Cash Flow', '2021 Free Cash Flow',
                          '2020 Free Cash Flow', '2019 Free Cash Flow', '2018 Free Cash Flow', '2017 Free Cash Flow',
                          'TTM+1 Free Cash Flow', 'Valid Ticker', 'Next Earnings Date', 'Date']
            csv_data = []
...
def Save_List(list):
    ...
    for company in list:
        try:
            ...
            print(str(round(100 * (list.index(company) / len(list)), 2)), "% done, about ", minutes, " minutes:",
...
def Calculate_Financial_Averages(df):
    ...
    df['Average Revenue 3 Years'] = df.loc[:, 'TTM Revenue':str(TTM_2) + " " + 'Revenue'].mean(axis=1,
                                                                                               numeric_only=True)
    df['Median Revenue'] = df.loc[:, 'TTM Revenue':str(TTM_5) + " " + 'Revenue'].median(axis=1, numeric_only=True)

    df['Average Net Income Margin'] = df.loc[:, 'TTM Net Income':str(TTM_5) + " " + 'Net Income'].mean(axis=1,
                                                                                                       numeric_only=True)
    df['Average Net Income Margin 3 Years'] = df.loc[:, 'TTM Net Income':str(TTM_2) + " " + 'Net Income'].mean(axis=1,
                                                                                                               numeric_only=True)
    df['Median Net Income Margin'] = df.loc[:, 'TTM Net Income':str(TTM_5) + " " + 'Net Income'].median(axis=1,
                                                                                                        numeric_only=True)

    df['Average Free Cash Flow'] = df.loc[:, 'TTM Free Cash Flow':str(TTM_5) + " " + 'Free Cash Flow'].mean(axis=1,
                                                                                                            numeric_only=True)
    df['Average Free Cash Flow 3 Years'] = df.loc[:, 'TTM Free Cash Flow':str(TTM_2) + " " + 'Free Cash Flow'].mean(
        axis=1, numeric_only=True)
    df['Median Free Cash Flow'] = df.loc[:, 'TTM Free Cash Flow':str(TTM_5) + " " + 'Free Cash Flow'].median(axis=1,
                                                                                                             numeric_only=True)
    return df
...
def Calculate_Financial_Margins(df):
    ...
    df['Average Net Income Margin'] = df.loc[:, 'TTM Net Income Margin':'TTM-5 Net Income Margin'].mean(axis=1,
                                                                                                        numeric_only=True)
    df['Average Net Income Margin 3 Years'] = df.loc[:, 'TTM Net Income Margin':'TTM-2 Net Income Margin'].mean(axis=1,
                                                                                                                numeric_only=True)
    df['Median Net Income Margin'] = df.loc[:, 'TTM Net Income Margin':'TTM-5 Net Income Margin'].median(axis=1,
                                                                                                         numeric_only=True)
    ...
    df['Average Free Cash Flow Margin'] = df.loc[:, 'TTM Free Cash Flow Margin':'TTM-5 Free Cash Flow Margin'].mean(
        axis=1, numeric_only=True)
    df['Average Free Cash Flow Margin 3 Years'] = df.loc[:,
                                                  'TTM Free Cash Flow Margin':'TTM-2 Free Cash Flow Margin'].mean(
        axis=1, numeric_only=True)
    df['Median Free Cash Flow Margin'] = df.loc[:, 'TTM Free Cash Flow Margin':'TTM-5 Free Cash Flow Margin'].median(
        axis=1, numeric_only=True)
    # free cash flow to net income
    df['TTm Free Cash Flow To Net Income'] = df['TTM Free Cash Flow'] / df['TTM Net Income']
    df['TTm-1 Free Cash Flow To Net Income'] = df[str(TTM_1) + " " + 'Free Cash Flow'] / df[
        str(TTM_1) + " " + 'Net Income']
    df['TTm-2 Free Cash Flow To Net Income'] = df[str(TTM_2) + " " + 'Free Cash Flow'] / df[
        str(TTM_2) + " " + 'Net Income']
    df['TTm-3 Free Cash Flow To Net Income'] = df[str(TTM_3) + " " + 'Free Cash Flow'] / df[
        str(TTM_3) + " " + 'Net Income']
    df['TTm-4 Free Cash Flow To Net Income'] = df[str(TTM_4) + " " + 'Free Cash Flow'] / df[
        str(TTM_4) + " " + 'Net Income']
    df['TTm-5 Free Cash Flow To Net Income'] = df[str(TTM_5) + " " + 'Free Cash Flow'] / df[
        str(TTM_5) + " " + 'Net Income']
    df['Average Free Cash Flow To Net Income'] = df.loc[:,
                                                 'TTm Free Cash Flow To Net Income':'TTm-5 Free Cash Flow To Net Income'].mean(
        axis=1, numeric_only=True)
    df['Average Free Cash Flow To Net Income 3 Years'] = df.loc[:,
                                                         'TTm Free Cash Flow To Net Income': 'TTm-2 Free Cash Flow To Net Income'].mean(
        axis=1, numeric_only=True)
    df['Median Free Cash Flow To Net Income'] = df.loc[:,
                                                'TTm Free Cash Flow To Net Income':'TTm-5 Free Cash Flow To Net Income'].median(
        axis=1, numeric_only=True)
    return df
...
def Calculate_Growth_Rates(df):
    ...
    df['Average Revenue Growth Rate'] = df.loc[:, 'TTM Revenue Growth Rate':'TTM-4 Revenue Growth Rate'].mean(axis=1,
                                                                                                              numeric_only=True)
    df['Average Revenue Growth Rate 3 Years'] = df.loc[:, 'TTM Revenue Growth Rate':'TTM-2 Revenue Growth Rate'].mean(
        axis=1, numeric_only=True)
    df['Median Revenue Growth Rate'] = df.loc[:, 'TTM Revenue Growth Rate':'TTM-4 Revenue Growth Rate'].median(axis=1,
                                                                                                               numeric_only=True)
    ...
    df['Average Net Income Growth Rate'] = df.loc[:, 'TTM Net Income Growth Rate':'TTM-4 Net Income Growth Rate'].mean(
        axis=1, numeric_only=True)
    df['Average Net Income Growth Rate 3 Years'] = df.loc[:,
                                                   'TTM Net Income Growth Rate':'TTM-2 Net Income Growth Rate'].mean(
        axis=1, numeric_only=True)
    df['Median Net Income Growth Rate'] = df.loc[:, 'TTM Net Income Growth Rate':'TTM-4 Net Income Growth Rate'].median(
        axis=1, numeric_only=True)
    # Free Cash Flow Growth rate
    df['TTM Free Cash Flow Growth Rate'] = df['TTM Free Cash Flow'] / df[str(TTM_1) + " " + 'Free Cash Flow']
    df['TTM-1 Free Cash Flow Growth Rate'] = df[str(TTM_1) + " " + 'Free Cash Flow'] / df[
        str(TTM_2) + " " + 'Free Cash Flow']
    df['TTM-2 Free Cash Flow Growth Rate'] = df[str(TTM_2) + " " + 'Free Cash Flow'] / df[
        str(TTM_3) + " " + 'Free Cash Flow']
    df['TTM-3 Free Cash Flow Growth Rate'] = df[str(TTM_3) + " " + 'Free Cash Flow'] / df[
        str(TTM_4) + " " + 'Free Cash Flow']
    df['TTM-4 Free Cash Flow Growth Rate'] = df[str(TTM_4) + " " + 'Free Cash Flow'] / df[
        str(TTM_5) + " " + 'Free Cash Flow']
    df['Average Free Cash Flow Growth Rate'] = df.loc[:,
                                               'TTM Free Cash Flow Growth Rate':'TTM-4 Free Cash Flow Growth Rate'].mean(
        axis=1, numeric_only=True)
    df['Average Free Cash Flow Growth Rate 3 Years'] = df.loc[:,
                                                       'TTM Free Cash Flow Growth Rate':'TTM-2 Free Cash Flow Growth Rate'].mean(
        axis=1, numeric_only=True)
    df['Median Free Cash Flow Growth Rate'] = df.loc[:,
                                              'TTM Free Cash Flow Growth Rate':'TTM-4 Free Cash Flow Growth Rate'].median(
        axis=1, numeric_only=True)
    return df
...
def Recursive_DCF(Initial_Free_Cash_Flow, Growth_Rate, Years, Current_Assets, Liabilities, Shares_Outstanding,
                  Current_Price, Dividend, Expected_Return=0):
    ...
    if Value_Per_Share > Current_Price:
        Expected_Return += 0.1
        return Recursive_DCF(Initial_Free_Cash_Flow, Growth_Rate, Years, Current_Assets, Liabilities,
                             Shares_Outstanding, Current_Price, Dividend, Expected_Return)
    else:
        return round((Expected_Return + Dividend * 0.75), 2)


def Calculate_DCF(df):
    df['DCF Revenue 5 Years'] = df.apply(
        lambda row: Recursive_DCF(Initial_Free_Cash_Flow=row['Average Free Cash Flow Margin'] * row['Average Revenue'],
                                  Growth_Rate=row['Average Revenue Growth Rate'], Years=6,
                                  Current_Assets=row['Current Assets'], Liabilities=row['Liabilities'],
                                  Shares_Outstanding=row['Shares Outstanding'], Current_Price=row['Current Price'],
                                  Dividend=row['Dividend'], Expected_Return=5), axis=1)
    df['DCF Revenue 3 Years'] = df.apply(lambda row: Recursive_DCF(
        Initial_Free_Cash_Flow=row['Average Free Cash Flow Margin'] * row['Average Revenue 3 Years'],
        Growth_Rate=row['Average Revenue Growth Rate 3 Years'], Years=6, Current_Assets=row['Current Assets'],
        Liabilities=row['Liabilities'], Shares_Outstanding=row['Shares Outstanding'],
        Current_Price=row['Current Price'], Dividend=row['Dividend'], Expected_Return=5), axis=1)
    df['DCF Net Income 5 Years'] = df.apply(lambda row: Recursive_DCF(
        Initial_Free_Cash_Flow=row['Average Free Cash Flow To Net Income'] * row['Average Net Income Margin'] * row[
            'Average Revenue'], Growth_Rate=row['Average Revenue Growth Rate'], Years=6,
        Current_Assets=row['Current Assets'], Liabilities=row['Liabilities'],
        Shares_Outstanding=row['Shares Outstanding'], Current_Price=row['Current Price'], Dividend=row['Dividend'],
        Expected_Return=5), axis=1)
    df['DCF Net Income 3 Years'] = df.apply(lambda row: Recursive_DCF(
        Initial_Free_Cash_Flow=row['Average Free Cash Flow To Net Income'] * row['Average Net Income Margin'] * row[
            'Average Revenue 3 Years'], Growth_Rate=row['Average Revenue Growth Rate'], Years=6,
        Current_Assets=row['Current Assets'], Liabilities=row['Liabilities'],
        Shares_Outstanding=row['Shares Outstanding'], Current_Price=row['Current Price'], Dividend=row['Dividend'],
        Expected_Return=5), axis=1)
    df['DCF Free Cash Flow 5 years'] = df.apply(
        lambda row: Recursive_DCF(Initial_Free_Cash_Flow=row['Average Free Cash Flow'],
                                  Growth_Rate=row['Average Revenue Growth Rate'], Years=6,
                                  Current_Assets=row['Current Assets'], Liabilities=row['Liabilities'],
                                  Shares_Outstanding=row['Shares Outstanding'], Current_Price=row['Current Price'],
                                  Dividend=row['Dividend'], Expected_Return=5), axis=1)
    df['DCF Free Cash Flow 3 years'] = df.apply(
        lambda row: Recursive_DCF(Initial_Free_Cash_Flow=row['Average Free Cash Flow 3 Years'],
                                  Growth_Rate=row['Average Revenue Growth Rate'], Years=6,
                                  Current_Assets=row['Current Assets'], Liabilities=row['Liabilities'],
                                  Shares_Outstanding=row['Shares Outstanding'], Current_Price=row['Current Price'],
                                  Dividend=row['Dividend'], Expected_Return=5), axis=1)
    df['Average Expected Return'] = round(
        df.loc[:, 'DCF Revenue 5 Years':'DCF Free Cash Flow 3 years'].mean(axis=1, numeric_only=True), 2)
    df['Median Expected Return'] = round(
        df.loc[:, 'DCF Revenue 5 Years':'DCF Free Cash Flow 3 years'].median(axis=1, numeric_only=True), 2)
    return df
